...
function Scene(_canvas) {
    ...
    this.draw = function () {
        ...
        function getDistance(v1, v2) {
            let detaX = v1.x - v2.x, detaY = v1.y - v2.y, detaZ = v1.z - v2.z;
            return Math.sqrt(detaX*detaX + detaY*detaY + detaZ*detaZ);
        }

        function getBase(v1, v2, range) {
            let baseX =( v1.x + v2.x ) * 0.5;
            let baseY = ( v1.y + v2.y ) * 0.5;
            let baseZ = ( v1.z + v2.z) *0.5;
            return {x:baseX - range/2, y:baseY - range/2, z:baseZ - range/2};
        }


        let shouldDrawBalls = [];
        for(let i = 0; i != users.length; ++i){
            shouldDrawBalls.push(true);
        }

        for(let i = 0; i != users.length; ++i){
            for(let j = i + 1; j != users.length; ++j){
                let user1 = users[i], user2 = users[j];
                let distance = getDistance(user1.pos, user2.pos);
                let k = 0.75;
                if(distance*k < user1.radius + user2.radius){
                    let range = ( user1.radius + user2.radius) / k * 2;
    ...
    this.textureBorder = new TextureBorder(config.sceneRadius, './scripts/resources/background.jpg', gl, this.textureShaderProgram)
    this.userBalls = []
    this.foodBalls = []
...
let viewDistance = config.cameraInitDistance
...
function onMouseWheel(e) {
    ...
    viewDistance -= deltaY
    if (viewDistance < currentUser.radius * config.cameraMinRatio) {
        viewDistance = currentUser.radius * config.cameraMinRatio
    } else if (viewDistance > config.cameraMaxDistance) {
        viewDistance = config.cameraMaxDistance
    }
}
